...
final class MascotController {
    ...
    func recordTextInput(at inputTime: TimeInterval? = nil) {
        behavior.recordTextInput(at: inputTime ?? Self.now)
        refresh()
    }
    ...
    func requestNoticeResult() {
        behavior.requestFeedback(.noticeResult, at: Self.now)
        refresh()
    }
