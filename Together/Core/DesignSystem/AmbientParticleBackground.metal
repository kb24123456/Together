...
[[ stitchable ]] half4 ambientParticleField(
    ...
    for (int layer = 0; layer < 4; layer += 1) {
        ...
        // Light mode uses fewer, clearer particles on the white canvas while
        // dark mode preserves its existing 16% active-cell distribution.
        float densityThreshold = mix(0.88, 0.84, darkAppearance);
        if (densityNoise > densityThreshold) {
            ...
            float darkMinimumRadiusPoints = mix(0.40, 0.60, nearWeight);
            float darkMaximumRadiusPoints = mix(0.70, 1.10, nearWeight);
            float lightMinimumRadiusPoints = mix(0.50, 0.65, nearWeight);
            float lightMaximumRadiusPoints = mix(0.78, 1.10, nearWeight);
            float minimumRadiusPoints = mix(
                lightMinimumRadiusPoints,
                darkMinimumRadiusPoints,
                darkAppearance
            );
            float maximumRadiusPoints = mix(
                lightMaximumRadiusPoints,
                darkMaximumRadiusPoints,
                darkAppearance
            );
    ...
    float alpha = min(accumulatedAlpha, maximumAlpha)
        ...
        * bottomFade
        * contrastMultiplier;
    half3 particleColor = mix(half3(0.32, 0.36, 0.42), half3(0.96, 0.95, 0.93), half(darkAppearance));
