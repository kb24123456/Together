...
- 通用协作方式沿用全局 Codex 规范；本文件补充 Together 特有的产品、工程和验收约束。
...
## 3. 按任务读取文档
- 首次接触项目或处理跨模块任务时，先了解 `PRODUCT_SPEC.md` 的当前定位与范围、`DEVELOPMENT_GUIDELINES.md` 的相关工程约束，以及当前 `AGENTS.md`；涉及 UI 时读取 `DESIGN_GUIDELINES.md` 对应章节。
- 局部修改只读取相关规范和 `Features / Models / Services` 文件；复杂任务按需查阅 `docs/PROJECT_MEMORY.md` 的相关记录。已读且未变化的内容不重复读，不为小改动重读全部主文档或扫描全仓。
...
## 6. 执行边界
- 新增或重构 UI、交互、动效时按第 4 节判断是否需要确认；OCR 保留“草稿确认后入库”的产品约束，具体设计沿用已确认方向。
...
- 核心数据模型、状态机、信息架构、第三方依赖、用户数据迁移的新方案或范围变化必须先确认；已明确授权的方案内自主实现与验证。既有产品决策保留：允许删除旧 Supabase 数据、不保留 RevenueCat、不保留 Supabase webhook、CloudKit 作为唯一跨设备同步/恢复能力；这不构成任意删除当前用户数据的授权。
...
- 复杂任务按第 3 节读取相关主文档及项目记忆，先核实现行事实；无需重复读取无关历史记录。
