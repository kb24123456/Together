...
struct Story: Decodable {
    ...
    var events: [Event]
    var startupSteps: [Double]?
    var firstFrameAdvance: Double?
}
...
@main struct SequenceProbe {
    @MainActor static func main() async throws {
        ...
        // Exact app load sequence: write initial properties, queue fixed advance commands,
        // then await a value read as the Worker barrier before publishing the Rive instance.
        for event in eventsByFrame[0] ?? [] {
        ...
        for dt in story.startupSteps ?? [] { machine.advance(by: dt) }
        _ = try await instance.value(of: NumberProperty(path: "mode"))
        ...
        for frame in 0..<story.frames {
            ...
            machine.advance(by: frame == 0 ? (story.firstFrameAdvance ?? 1.0 / Double(fps)) : 1.0 / Double(fps))
        ...
        var startupTrace: [[String:Any]] = []
        for dt in story.startupSteps ?? [] {
            _ = inspectionMachine.advance(by:dt)
            startupTrace.append(["dt":dt,"states":inspectionMachine.stateChanges()])
        }
        ...
        for frame in 0..<story.frames {
            ...
            _ = inspectionMachine.advance(by:frame == 0 ? (story.firstFrameAdvance ?? 1.0/Double(fps)) : 1.0/Double(fps))
        ...
        try writeJSON([
            ...
            "startupSteps":story.startupSteps ?? [],"startupTrace":startupTrace,"firstFrameAdvance":story.firstFrameAdvance ?? 1.0/Double(fps),
