...
3. 增加 `sendTaskComment` 应用服务。
4. 补 `task_messages` pull/catch-up。
5. 新增 `TaskChatViewModel` 和 timeline 聚合模型。
6. 实现 `TaskChatPanelView`。
7. 接入 `PairTimelineCard` 留言入口和 latest comment preview。
8. 补未读状态。
9. 做真机动画、键盘、推送回归。
