...
已有任务展开采用“准备接管 + 真实行高 + 真实视口位移”的双向外撑。打开事件先在无动画 transaction 中让同一 UUID 的容器取得活跃所有权、保持 `.compact` 几何，并预挂载裁切为零高的详情；下一次渲染才以统一 spring 执行 `p: 0 -> 1`。卡片背景在准备态已经是实体，不参与整体 opacity 淡入；圆角、阴影、内边距、详情裁切和外部呼吸间距随 `p` 展开。关闭保持所有权不变并用同一端点执行 `p: 1 -> 0`，到达紧凑端点后才原子释放，因此是展开的严格逆向。
...
UIKit 桥接只负责解析当前 SwiftUI `ScrollView` 对应的底层 `UIScrollView` 并执行无附加动画的 O(1) offset 写入，不拥有任务视图、详情表面、业务状态或生命周期。逐帧 progress 只存在于活动行的 `AnimatableModifier`，不得写入首页级 `@Observable` / `@State`、不得广播给所有列表行，也不新增常驻 `CADisplayLink`。因此列表仍保持稳定 UUID、`LazyVStack` 懒加载与原生 pinned header；真机 60/120Hz 结果仍以 Instruments 和视觉验收为准。
...
展开期间外层列表保持可滚动，不增加详情内嵌滚动。非活跃任务的完成框、Swipe、菜单以及 Dock 操作暂停；点击另一任务主体只请求保存并收起当前任务，不自动打开第二项。Toolbar、列表、安全区与 Dock 布局保持稳定，不统一缩放、模糊或暗化首页；活跃容器只使用轻微实体抬升、连续圆角和克制阴影。行高、内容揭示、真实外部呼吸间距与 Morph 引入的视口位移必须共享活动行的同一个显式 presentation progress；内部编辑内容不得再启动第二套异步高度或整组透明度动画。
...
- 删除 `HomeFocusPresentationController`、`HomeFocusPresentationCoordinator`、`HomeFocusSurfaceView`、`HomeRootContainer`、全局 blur/dim 与详情 Overlay。
