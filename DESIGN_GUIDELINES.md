...
结论：
- 当前项目默认视觉基线固定使用 `mobile-02-cleanminimal_light`。
- 后续首页、一级导航页、核心内容页的视觉设计与 SwiftUI 落地，默认都必须以该 style guide 为统一参照。

事实：
- 该 style guide 对应的核心气质是：浅色基底、克制留白、圆角卡片、低饱和色彩、轻层次、弱阴影、简洁排版。
- 该 style guide 不是“可选参考”，而是当前项目的默认设计母版。
...
不得偏离项：
- 整体必须保持 light、clean、minimal 的基调
- 不允许转向厚重拟物、强发光、高饱和、大面积渐变堆叠
- 不允许出现过深阴影、过强描边、过度装饰性背景
- 不允许为了做“情侣感”而引入廉价粉色化、贴纸化、幼态化视觉语言
- 圆角、留白、字体节奏、弱分隔的整体关系必须稳定统一
...
6. 英文与数字默认使用 Rounded 字体设计。
事实：
- 产品内出现的英文品牌名、英文标签、日期数字、时间数字、计数数字，默认使用 rounded 字体风格。
...
- 事项卡片默认采用统一信息骨架：
  ...
  - 备注：标题下方
  - 执行者头像：右下
- 事项卡片高度必须由内容高度决定，禁止写死固定高度。
- 标题必须限制为 `1` 行；超出使用截断，不允许挤压备注和底部头像区。
- 备注最多 `2` 行；为空时卡片应自动收缩，不得保留空白占位。
- 卡片宽度必须由屏幕可用宽度与页面水平间距共同决定：
  - 优先使用 `maxWidth: .infinity` 或容器填充宽度
  - 不允许按固定机型宽度写死卡片宽度
- 粉色强调卡与普通卡必须共用同一排版骨架，只允许在颜色和轻微材质层级上区分，不允许出现两套完全不同的信息布局。
- 深色模式下：
  - 强调卡：黑色背景 + 分层粉色文字系统
  - 普通卡：黑色背景 + 暖灰白文字系统
  - 禁止直接使用“纯黑底 + 纯白全量文字”的粗暴反相方案
- 卡片本体可使用极轻阴影与极弱高光，但不得使用明显模糊、重发光或厚重投影。
