...
- 首页底部入口必须由 `NavigationStack` 的系统 `.bottomBar` 持有，只使用原生 `ToolbarItem`、`ToolbarSpacer`、`Menu`、`Button` 与 SF Symbols；不得再用 `safeAreaInset`、自绘材质、圆形背景、阴影或额外几何状态复制 Dock。`+` 不采集全局 Frame，也不承担创建表面身份。创建 Overlay 首帧使用完整几何并立即请求标题焦点，底部始终锚定在键盘安全区上方 `16pt`；纵向位移只由 SwiftUI 的系统键盘安全区布局驱动，不叠加自定义 Spring、键盘通知、逐帧高度写入或缩放。根表面统一使用 iOS 26 `ConcentricRectangle` 继承屏幕 / 容器圆角语义，并让四角使用统一解析半径；待办与定期内容都按自然高度测量，不写死定期面板高度。卡片外透明命中层调用同一 `requestDismissal()`，卡片内容层位于其上并拦截内部点击；保存阶段由会话 phase 拒绝 dismiss。创建卡出现、存在和退出期间，根页面保持原始视觉，不绘制 scrim，也不得对任务行、Header、顶部或底部 Toolbar、根背景施加创建态 opacity、brightness、scale、blur 及其反向恢复动画。编辑器在创建会话后的下一渲染阶段挂载并申请输入焦点；首次及后续内容高度测量只在数值实际变化时写入，并显式禁用动画。表面仍只做约 `140ms` 淡入；退出时完整卡片约 `220ms` 向屏幕下边缘滑出；Reduce Motion 使用约 `140ms` 表面淡变并取消大幅位移。已有任务详情保持连续列表背景，不使用圆角卡、描边、阴影、屏幕级暗化或全屏 / 几何驱动模糊；非活跃任务允许使用上述有界、离散距离模糊，并临时成为“保存并收起”命中层。
