...
- 只要实现自定义动画，必须由单一交互会话和显式状态驱动。已有任务详情将真实行高、紧凑内容高度、身份组 X/Y/尺度、交叉淡变与详情级联时间轴拆成独立 Keyframe track：身份组在约 `520ms` 内沿内部轨迹 `(0, 0) -> (-7pt, +14pt) -> (-16pt, -12pt)` 的连续曲线直接到达约 `1.12` 视觉尺度，Y 轴端点补偿展开态新增的 `14pt` 顶部真实间距，使屏幕下潜 / 上升接近等幅；不追加 Overshoot、Spring 或终点回落。详情视觉行从共享时间轴派生 `(+14pt, +24pt) -> (+5pt, +9pt) -> (0, 0)` 曲线和透明度，首行最早约 `540ms` 落定，禁止早于身份组，整组最迟约 `800ms` 完成。收起使用约 `360ms` 的精确反向关键帧，详情级联直接从同一 `layoutProgress` 派生，避免独立时间轴重置导致内容瞬隐；从底部属性开始逆序退出，真实行高不得提前归零。收起结束由单一、可取消且校验会话 token 的生命周期任务驱动，不观察动画帧阈值。禁止用多个 `Task.sleep` 拼接视觉阶段，禁止把逐帧进度发布到根级 Observation，也禁止用 `CADisplayLink` 或连续高度监听向 SwiftUI 状态逐帧写值。
...
- 已有任务详情完全由 SwiftUI 列表拥有：`TaskMorphContainer` 只改变当前行的真实高度和局部合成属性，不解析或逐帧写入 `UIScrollView.contentOffset`，不主动滚动、不复制任务表面。活跃区域整体执行约 `1.03` 合成缩放并用上下各 `14pt` 的真实布局间距撑开相邻内容；非活跃任务以稳定任务序号与焦点行的有符号差值派生约 `0.92...0.86` scale、`0.40` opacity、向焦点外侧 `4...12pt` offset 和 `1.3...4.8pt` 有界模糊，并使用每级约 `25ms`、最多 `120ms` 的内建动画延迟形成水波传播。日期 / 周期 Header 只使用约 `1.6pt` 轻度模糊。所有帧均由 `.scaleEffect / .offset / .blur / .opacity` 合成插值，不读取连续 frame、不向 Environment / Observation 发布滚动坐标；详情期间底部边缘流场回到恒等值。只有当前活跃行可测量紧凑 / 展开文本与详情自然高度；普通行不得长期运行几何测量。待办与定期使用同一静态轴线、属性胶囊和单行横向属性轨道，不因字段不同复制动效状态机。
