...
- 只要实现自定义动画，必须由单一交互会话和显式状态驱动；同一次焦点转场的真实行高、视口位移、内容揭示和背景景深服从一条无过冲的主视觉进度。活动表面外壳允许从同一展开目标派生可取消的局部视觉阶段，在主几何落定附近明确超过最终边界一次，再带阻尼回落；该阶段只能改变外壳绘制范围与阴影，不得改变固定圆角、反向驱动真实布局、内容坐标或业务状态，也不得拥有业务完成回调。收起时直接从当前呈现值改向紧凑端点，不得先重播向外过冲；禁止用多个互不知情的动画与临时业务状态拼接。
...
- 首页焦点动效采用列表拥有的窄混合边界：SwiftUI 持有真实时间线、唯一 `TaskMorphContainer`、编辑内容、业务状态和持久化；UIKit 只解析底层 `UIScrollView` 并根据活动行的单一显式进度写入无附加动画的 content offset，不持有任务表面、背景遮罩、业务状态或生命周期。不得让 SwiftUI 和 UIKit 同时修改同一受 SwiftUI 布局管理视图的 Frame。
- 自定义底部 Dock 必须始终挂载并保留固定安全区高度。非 Reduce Motion 下，`+` 只作为到真实 Draft Seed 的短暂 Hero 来源；顶部系统 Toolbar 保留导航职责，不作为 Morph 参与者。活动行是详情阶段唯一圆角表面，不新增屏幕级暗化、动态模糊、卡片形镂空、重复前景卡或透明占位；详情关闭与创建落地共用可中断的 `1 -> 0` 任务表面语法，持久化提交是唯一锁定冲突交互的阶段。
