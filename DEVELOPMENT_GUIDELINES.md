...
- 已有任务详情由根级 `fullScreenCover` 打开，并在模态内拥有独立 `NavigationStack`。列表行保持紧凑布局，以 `TaskDetailRoute` 和稳定 UUID 标记行内实际内容宽度的 matched transition source；目的页使用单一 `ScrollView`、系统语义背景和详情专用全屏编辑组件，不得把 `HomeTimelineRow`、`HomeInlineTaskDetailCard`、`RoutinesTaskRow` 的展开态、Morph 测量区或 cascade 布局直接作为页面正文。不解析或写入 `UIScrollView.contentOffset`，不主动滚动、不复制任务表面、不改变其他任务的 scale / blur / opacity。进入目的页前由对应 ViewModel 建立唯一编辑 Draft；View 只持有输入、焦点、当前编辑子任务等局部视觉状态，业务 Draft、保存和完成语义继续由 ViewModel / 应用服务承载。标题编辑器首帧稳定挂载，页面挂载后一帧立即请求 `FocusState`，不等待 `viewDidAppear`、不使用固定延时或键盘通知。顶部叉号、VoiceOver Escape 与系统左滑交互式关闭直接放弃尚未确认的 Draft；右上角勾号只在标题非空且标题、备注、子任务或属性 Draft 相对已保存任务发生实际变化时启用，并在提交前统一刷新标题、备注、现有 / 新增子任务的输入法组合文本。保存 / 完成关闭期间通过 `interactiveDismissDisabled` 阻止并发关闭，保存成功才关闭模态，失败留在当前页；完成动作同样先刷新输入并保存。待办与定期底部属性轨道都使用 `safeAreaBar`，每个胶囊按固有内容宽度保留至少 `8pt` 水平内边距，整组可放下时自然排列，不足时由 `ViewThatFits` 回退为单行横向滚动；不得用可压缩的等分 frame 包裹固定宽度文字。待办日期 / 时间 / 提醒按各入口使用轻量 Sheet，定期周期 / 目标日 / 目标时间 / 提醒使用一个轻量 Sheet 宿主，按冻结的入口直接显示日期、时间或提醒内容，并只更新当前编辑 Draft。每个入口使用独立、稳定的属性 source ID，`matchedTransitionSource` 只包裹 `34pt` 可见胶囊而不包含外围 `44pt` 命中区。点击时必须把输入焦点状态与 source 一并冻结到本次 presentation：没有输入焦点且未开启 Reduce Motion 时，Sheet 根视图按本次来源使用系统 `.navigationTransition(.zoom)` 打开并返回；存在输入焦点或开启 Reduce Motion 时，本次打开和关闭成对使用 `.automatic`，避免胶囊随键盘安全区下落后再从错误位置 Zoom。不得在 Sheet 已呈现后切换转场策略，也不得用键盘通知、高度读取、固定延时、`matchedGeometryEffect` 或逐帧几何复制该效果。普通字号的紧凑 detent 必须在 Sheet 根层按实际内容高度计算，辅助功能字号改用可滚动 `.large`，不得把 presentation sizing 只挂在条件分支内。定期目标日 JSON 规则允许月度 `weekdayOfMonth` 与月 / 季 / 年 `lastBusinessDay`，新增 case 不修改 SwiftData 字段；计算器必须约束候选日期仍位于当前周期，切换周期时 ViewModel 统一归一化或清除不兼容规则。
...
- 已有待办详情的时间入口使用 `TaskTimePickerSheet` 紧凑系统 Sheet，其余入口沿用既有容器，不建立全局独立 Picker 或专用大号 Sheet。刻度使用系统横向 ScrollView、居中 view-aligned 吸附与固定指针；连续位置、基础亮度和球面放大的 scaleEffect / 水平 offset 通过 visualEffect 处理，不改变布局或吸附几何；TaskTimeRulerLens 按距固定指针的连续距离返回 1–1.24 倍升余弦放大，水平 offset = distance × (scale − 1)，让刻线尺寸与视觉间距协同变化、水平中线保持稳定，接触变化局部动画，滚动坐标不写入 State；onScrollPhaseChange 的 tracking / interacting 仅控制局部接触态，不因此写入 Draft。每根 TaskTimeRulerMark 通过 onGeometryChange 发布左侧 / 中心 / 右侧三态，由各自的 TaskTimeRulerLightState 触发局部 keyframeAnimator，跨帧跳过中心也要照亮，余光不得重新乘固定中心距离导致拖尾消失。松手令球面放大恢复并停止新提亮，既有余光自然收尾；提亮仅叠加最高 18% 白色，不能淡掉底线。清除、程序定位、打开系统滚轮及失活重置光层身份，不能重建滚动节点。选择值仍由节点“是否在指针最近半格内”的 Bool 更新，不向状态发布逐帧偏移量。打开时滚动定位不写 Draft，只有用户拖动、点选或系统滚轮的 valueChanged 可产生选择；清除和程序定位必须屏蔽残余滚动回调。刻度由 `TaskTimeRulerScale` 按所选 Calendar 自然日生成，排除不存在的夏令时时刻，重复时刻使用第一次；不得按固定 24 小时秒数推进日期。仍复用 `ExistingTaskScheduleDraft` 与 `HomeViewModel.updateDraftSchedule`，不新增持久化或提交状态机。定期详情目标时间通过 `PeriodicTimePickerValue` 复用同一 `TaskTimePickerSheet`；适配层用中性日历表达完整 24 小时墙钟值，只将 hour / minute 写回 RoutinesViewModel，避免当天夏令时缺口误删未来重复任务可选时刻。待办日期时间 Sheet 继续使用系统 compact `DatePicker`；OCR 统一属性 Sheet 继续使用既有 `TaskEditorSingleColumnTimeWheel`，原内联入口继续使用小型系统 wheel Popover。所有入口共享 5 分钟粒度、清除时间同步清除提醒及原提醒提前量语义；不得再接入大号双组件 `UIPickerView`、双 `UITableView` 圆柱滚轮、边缘模糊层或另一套 pending 提交状态机。

- 时间刻度高光、边缘柔化和选值统一基于外层命名视窗空间。居中首尾留白必须放在 scrollTargetLayout 内容的 padding 内，不能用 contentMargins 在初次 scrollPosition 定位后才补入留白；隔离复现显示后者可让初始位置偏约半个视窗。所有刻线基础尺寸与间隔一致，球面形变只作用绘制层；分别执行自身 visualEffect，不能按一对主次刻线共享亮度；边缘柔化和选中瞬态柔光不增加背景模糊层。数字使用 numericText(value:) 并在 Text 局部绑定 selectedTime 动画，不能只声明 contentTransition 而没有动画事务，也不能让数字动画影响刻度布局。节点之间的视觉分割线不得新增选择值或改变 5 分钟吸附精度。Sheet detent 测量实际内容高度，外缘 inset = max(24, bottomSafeArea)，内容底部仅补不足部分；按钮使用 ConcentricRectangle，大时间通过 UIFont/CoreText 的稳定 00:00 样本去掉字体空白，以 containerCornerOffset 避让圆角，保留原生 Text 与 numericText，不裁剪数字或增加底框。

- 定期日期选择的选项映射集中于 `PeriodicSchedulePickerValues`，横滑与程序定位属于 `PeriodicDayStrip` 的局部展示状态。居中节点只发布 Bool，visualEffect 只改变明暗，不发布逐帧 offset、不缩放日期。仅用户拖动 / 点击 / VoiceOver 调整写编辑 Draft；打开、周期重建、菜单定位和清除期间屏蔽滚动回写。月度旧 dayOfPeriod(31) 仅映射到月末显示，未操作前不改存储规则；特殊历史规则必须保留为有效选项。活跃 Draft 存在且规则为空时，不能以空值回退到持久化任务的旧规则。

- 定期日期带的月 / 季数字采用视窗七等分槽宽，至少 44pt 并适配字号，同一视窗内位数变化不改变槽宽。有限日期集使用 HStack 以精确测量可变视窗下的内容宽度，避免 LazyHStack 估宽残留导致边界偏移。ScrollPosition 在首次有效布局及视窗 / 字号变化时显式按中心重新定位；不移动箭头、不重建滚动容器，程序定位不回写 Draft。年度五个快捷值仍由 PeriodicSchedulePickerValues 提供，界面直接调用既有 Draft 更新方法；既有非快捷规则只回显、不主动替换。
