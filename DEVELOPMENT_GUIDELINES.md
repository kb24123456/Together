...
- 待办与定期列表删除复用一个局部 `TaskDeletionAnimationSession`：触觉后立即调用 repository / application service，持久化失败只恢复空闲态，成功后才用 SwiftUI `layerEffect` 驱动约 `1.8s` 的文字粒子化，并在约 `220ms` 的 `LazyVStack` 真实移除事务中收拢。SwiftUI 侧使用线性总时钟，释放前沿、约 `0.68...1.18pt` 的采样单元、约 `0.5...1.1pt` 的细尘覆盖、约占单颗寿命 `34%` 的渐进形成、临界阻尼位移、低频横风 / 纵向阵风和 `76%` 后才开始的尾段衰减全部由 shader 内的确定性函数派生；必须避免粒子尚未产生可见位移时先把原文字覆盖率降空。shader 只包裹文字区域，向右最大约 `70pt`、向上及扰动最大约 `22pt`，采样外扩限制为约 `72 × 24pt`；完成框等控件在约 `500ms` 内单独淡出缩小，不创建全屏合成层、CPU 粒子数组、第三方依赖或逐帧 Observation 写入。一次只允许一个删除会话；Reduce Motion、低电量、严重 / 临界热状态改用短淡出，模式 / 场景 / 视口中断直接完成并清理会话。
